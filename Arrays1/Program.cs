...
/*        static void CardShark()
        {
            ...
            Console.WriteLine("How much gil do you want to risk?");

            gilToRisk = int.Parse(Console.ReadLine());

            int nextCardIndex = RandomNumber(1, 52);
            int lastCardIndex = nextCardIndex;
            string lastCard = cards[lastCardIndex];
            string nextCard = cards[nextCardIndex];

            Console.WriteLine(cards[nextCardIndex]);

            while (bust == false && cardCount <= 10)
            {
                lastCardIndex = nextCardIndex;
                nextCardIndex = RandomNumber(1, 52);
                lastCard = cards[lastCardIndex];
                nextCard = cards[nextCardIndex];

                Console.WriteLine("Is the next card greater than (1) or less than (0) " + lastCard + "?");

                int greater = int.Parse(Console.ReadLine());

                int lastCardVal = int.Parse(lastCard.Substring(0, 1));
                int nextCardVal = int.Parse(nextCard.Substring(0, 1));

                if (greater == 1)
                {
                    if (lastCardVal < nextCardVal)
                    {
                        bust = true;
                    }

                    Console.WriteLine(lastCard + " is greater than " + nextCard);
                }
                else if (greater == 0)
                {
                    if (lastCardVal > nextCardVal)
                    ...
                    Console.WriteLine(lastCard + " is less than " + nextCard);
                }
